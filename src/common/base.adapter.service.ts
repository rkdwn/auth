...
import { BaseAdapterEntityDocument } from "./base.adapter.entity";

@Injectable()
export class BaseAdapterService {
  private name: string;
  ...
  async upsert(id: string, payload: Object, expiresIn: number): Promise<void> {
    try {
      ...
      console.error(`[ADAPTER] (upsert) ${JSON.stringify(e)}`);
    }
  }
   ...
   *
   */
  async findByUserCode(userCode: string): Promise<Object> {
    try {
      const _result = await this.model.findOne<BaseAdapterEntityDocument>(
   ...
   *
   */
  async findByUid(uid: string): Promise<Object> {
    try {
      const _result = await this.model.findOne<BaseAdapterEntityDocument>(
        { "payload.uid": uid },
        { payload: 1 },
